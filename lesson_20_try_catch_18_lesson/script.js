/* Добавить обработку ошибок в предыдущие задания. */

/* ИЗМЕНИЛ URL В ПЕРЕМЕННОЙ urlRandom, на несуществующий*/
...
const urlRandom = 'https://api.emojisworld.fr/v1/abra-cadabra';
...
/* ИЗМЕНИЛ URL В ПЕРЕМЕННОЙ url, на несуществующий*/
...
const url = 'https://restcountries.com/v3.1/top-site';
...
fetch(url)
  .then(response => {
    if(response.status === 404) {
      throw new Error ('NOT FOUND TOP SITE!!!!')
    }
    return response.json();
  })
...
/* ИЗМЕНИЛ URL В ПРОМИСАХ, на несуществующий*/
...
//     fetch('https://restcountries.com/v3.1/no-no-no-no')
//     .then(resp => {
//       if(!resp.ok) {
...
//REJECT
fetch('https://restcountries.com/v3.1/name/spain-not-found')
  .then(resp => {
    if(resp.status !== 200) {
      throw new Error('ALARM, SPAIN IS NOT FOUND')
    }
    return resp.json();
  })
  .then(data => {
